...
float CalcAlphaForCircle(float2 samplePosition, float innerRadius, float outerRadius){
    // samplePosition is expected to be in range [0, 1]
    // We translate it to [-0.5, 0.5] to center the circle
    float2 p = samplePosition - 0.5;
    float d = length(p);
    
    // Distance to outer circle (negative inside)
    float dOuter = d - outerRadius;
    // Distance to inner circle (negative outside)
    float dInner = innerRadius - d;
    
    // Max of both gives the distance to the ring
